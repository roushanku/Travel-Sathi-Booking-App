...
const ProductCard = () => {
  const { searchProduct } = useContext(UserContext); // Retrieve searchProduct from UserContext
  ...
  const totalPages = Math.ceil(searchProduct.length / ITEMS_PER_PAGE);
      ...
      {currentPageProducts && currentPageProducts.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
          {currentPageProducts.map((product, index) => (
            product ? (
              <div key={index} className="max-w-sm mx-auto bg-white rounded-lg shadow-lg overflow-hidden my-4 m-4">
                {product.photos.length > 0 ? (
                  <img className="w-full h-48 object-cover" src={product.photos[0]} alt="Image" />
                ) : (
                ...
                <div className="p-4">
                  ...
                  <p className="mt-2 text-gray-600">Address: {product.address}</p>
                  <p className="mt-2 text-gray-600">Description: {product.description}</p>
                  <p className="mt-2 text-gray-600">Price: ${product.price}</p>
                  <p className="mt-2 text-gray-600">Max Guests: {product.maxGuests}</p>
                  <button
              ...
              <div key={index}>No Hotels available</div>
            )
          ))}
        </div>
      ) : (
      ...
      {isModalOpen && selectedProduct && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg overflow-hidden shadow-lg max-w-md mx-auto p-6">
            <h2 className="text-2xl font-bold mb-4">Details of {selectedProduct.title}</h2>
            ...
            <p className="text-gray-700">Max Guests: {selectedProduct.maxGuests}</p>
            <button
